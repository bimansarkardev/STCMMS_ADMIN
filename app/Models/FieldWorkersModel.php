...
class FieldWorkersModel extends Model 
{
    ...
    function getAttendanceList($perPage, $filters = [])
    {
        //dd($filters);
        $query = FieldWorkersModel::query()
        ->leftJoin('user', 'user.user_id', '=', 'field_workers.user_id')
        ...
        ->leftJoin('roles', 'roles.id', '=', 'field_workers.role')
        ->leftJoin('agencies', 'agencies.id', '=', 'field_workers.agency_id')
        ->leftJoin('attendance', function ($join) use ($filters) {
            $join->on('attendance.field_worker_id', '=', 'field_workers.id');

            if (!empty($filters['currentDate'])) {
                $join->where('attendance.date', '=', $filters['currentDate']);
            }
        })
        ->select(
            'field_workers.id',
            ...
            'field_workers.agency_id',
            'agencies.agency_name',
            'attendance.created_by as attendance_created_by',
            'attendance.date as attendance_date',
            DB::raw("DATE_FORMAT(attendance.date, '%d/%m/%y') as formatted_attendance_date"),
